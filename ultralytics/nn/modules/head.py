...
class CustomRTDETRDecoder(nn.Module):
    ...
    def forward(self, x, batch=None):
        ...
        Args:
            x (List[torch.Tensor]): List of feature maps from the backbone.
            batch (dict, optional): Batch information for training. Contains GT
        ...
        print(f'dec_bboxes.shape: {dec_scores.shape}')
        print(f'dec_bboxes.shape: {enc_bboxes.shape}')
        print(f'dec_bboxes.shape: {enc_scores.shape}')
        print(f'dn_meta: {dn_meta}')
        x = dec_bboxes, dec_scores, enc_bboxes, enc_scores, dn_meta
        if self.training:
            return x
        # (bs, 300, 4+nc)
