...
class CRelu(nn.Module):
    ...
    def forward(self, x):
        """Apply CRelu activation function to input tensor."""
        return functional.relu(torch.cat([x, -1*x], 1))
...
class FirstConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        """Initialize Conv layer with given arguments including activation."""
        super().__init__()
        self.conv = nn.Conv2d(c1, int(c2/2) , k, s, autopad(k, p, d), groups=g, dilation=d, bias=False)
...
class HighPassConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        ...
        self.bn = nn.BatchNorm2d(int(c2/2))
        self.act = self.default_act if act is True else act if isinstance(act, nn.Module) else nn.Identity()
        self.channelsToCut = int(int(c2/2) * self.lowPToCut)

    def forward(self, x):
        """Apply convolution, batch normalization and activation to input tensor."""
        x = self.act(self.bn(self.conv(x)))
        if self.channelsToCut != 0:
            x =  x.permute(0, 2, 3, 1)
            lowestChannel = torch.argmin(x, dim = 3)
            x[lowestChannel] = 0
        return x
...
class SplitConv(nn.Module):
    ...
    def forward(self, x):
        ...
        texture = self.act(self.bn2(self.conv2(texture)))
        output = torch.cat([shapes, texture], dim=1)
        return output
